...
export interface ITreeSelectProps<T extends Model, TModelFilter extends ModelFilter> {
  mode: TreeSelectMode;
    ...
    const renderItemTree = React.useCallback(
      listData => {
        return listData.map((item: any) => (
          ...
          key={item.id}
          value={item.id}
          title={window.innerWidth < 1920 ?
            <Tooltip title={item.name}>{limitWord(item.name, 30)}</Tooltip>
            : <Tooltip title={item.name}>{limitWord(item.name, 55)}</Tooltip>}
